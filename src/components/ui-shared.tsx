...
export function ContactModal({ isOpen, onClose, productTitle }: { isOpen: boolean; onClose: () => void; productTitle: string }) {
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const text = `Hi, I am interested in ${productTitle}.\n\nName: ${name}\nMobile: ${mobile}\nEmail: ${email}\nAddress: ${address}`;
    const url = `https://wa.me/919693529897?text=${encodeURIComponent(text)}`;
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-background/80 backdrop-blur-sm">
      <div className="relative w-full max-w-md p-8 rounded-2xl bg-surface border border-border shadow-2xl">
        ...
        <h3 className="text-2xl font-display text-foreground mb-2">Get in touch</h3>
        <p className="text-sm text-muted-foreground mb-6">Fill out this form to inquire about {productTitle}.</p>
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          <input required name="name" type="text" placeholder="Name" className="w-full p-3 text-sm rounded-lg bg-background border border-border text-foreground outline-none focus:border-gold transition-colors" />
          <input required name="mobile" type="tel" placeholder="Mobile Number" className="w-full p-3 text-sm rounded-lg bg-background border border-border text-foreground outline-none focus:border-gold transition-colors" />
          <input required name="email" type="email" placeholder="Email" className="w-full p-3 text-sm rounded-lg bg-background border border-border text-foreground outline-none focus:border-gold transition-colors" />
          <textarea required name="address" placeholder="Address" className="w-full p-3 text-sm rounded-lg bg-background border border-border text-foreground min-h-[100px] outline-none focus:border-gold transition-colors" />
          <button type="submit" className="w-full p-4 rounded-lg bg-gold text-background font-bold mt-2 hover:bg-gold/90 transition-colors uppercase tracking-[0.2em] text-xs">Submit Inquiry</button>
        </form>
      </div>
    </div>
  );
...
export function Nav() {
  ...
  return (
    <>
      ...
      <ContactModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} productTitle="Avensis Services" />
    </>
  );
}
